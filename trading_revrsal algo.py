...
Algorithmic strategy template with a zero-profit reversal rule.
"""
...
class Strategy:
    ...
    def reset_state(self) -> None:
        ...
        self.trade_was_profitable = False # NEW: Track if the trade has been in profit
    ...
    def on_trade_update(
        ...
        if self.position != 0 and self.entry_price is not None:
            ...
            # First, check if the trade has ever been profitable
            if is_currently_profitable:
                self.trade_was_profitable = True

            # If the trade WAS profitable but is now at or below breakeven, reverse it
            if self.trade_was_profitable and not is_currently_profitable:
                print(f"ZERO-PROFIT REVERSAL triggered at {price:.2f}. Flipping position.")
                # To reverse, we place an order for double the size in the opposite direction
                ...
                # After reversing, we must wait for the on_account_update to fix our state
                return
    ...
    def on_account_update(
        ...
        """Called whenever an order is filled. Crucial for updating state."""
        ...
        # --- Logic to handle reversals and new entries ---
        # If position flipped sign (e.g., from +5 to -5), it was a reversal
        if (original_position > 0 and self.position < 0) or \
           (original_position < 0 and self.position > 0):
            print("Reversal confirmed. Updating entry price.")
            self.entry_price = price
            self.trade_was_profitable = False # Reset for the new trade
        # If we are entering a new position from flat
        elif original_position == 0 and self.position != 0:
            self.entry_price = price
            self.trade_was_profitable = False # Reset for the new trade
        # If we just closed a position to become flat
